...
func (d *dataTable[EI, E]) insert(id EI, data E) error {
	...
	if data2, ok := d.get(id); ok {
		return fmt.Errorf("multiple %Ts with the same ID: id=%d, data1=%v, data2=%v", data, id, data, data2)
	}
	d.sparse[id] = data
	return nil
}
...
func (s cpuSample) asEvent(table *evTable) Event {
	// TODO(mknyszek): This is v2-specific, but shouldn't be.
