...
func (w *recorder) Write(p []byte) (n int, err error) {
	...
	defer func() {
		// Propagate errors to the flightrecorder, stop tracing.
		if err != nil {
			if r.err == nil {
				r.err = err
			}
	...
	b, gen, err := readBatch(rd) // Is this guaranteed to be a complete batch?
	if err == io.EOF {
		if rd.Len() > 0 {
			return len(p) - rd.Len(), errors.New("short read") // Is this possible?
		}
		return len(p), nil
	}
	if err != nil {
