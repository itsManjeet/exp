...
type FlightRecorder struct {
	...
	writing bool // whether a WriteTo is in progress.
...
func (r *FlightRecorder) Start() error {
	...
	// Start tracing, sending data to the recorder goroutine.
...
func (r *FlightRecorder) WriteTo(w io.Writer) (total int, err error) {
	...
	if r.writing {
		return 0, fmt.Errorf("concurrent call to FlightRecorder.WriteTo")
	}
	r.writing = true
	defer func() {
		r.writing = false
...
func traceTimeNow() Time {
	...
	return Time(runtime_traceClockNow())
}
