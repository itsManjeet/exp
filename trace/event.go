...
func (s Stack) Frames(yield func(f StackFrame) bool) bool {
	...
	stk := s.table.stacks[s.id]
	for _, f := range stk.frames {
		sf := StackFrame{
			PC:   f.pc,
			Func: s.table.strings[f.funcID],
			File: s.table.strings[f.fileID],
			Line: f.line,
		}
		if !yield(sf) {
			return false
...
func (e Event) StateTransition() StateTransition {
	...
	case v2.EvGoStop:
		...
		s.Reason = v2.GoStopReason(e.base.args[0]).String()
	case v2.EvGoBlock:
		s = goStateTransition(e.ctx.G, GoRunning, GoWaiting)
		s.Reason = v2.GoBlockReason(e.base.args[0]).String()
	case v2.EvGoUnblock:
