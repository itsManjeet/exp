...
func (o *ordering) advance(ev *baseEvent, evt *evTable, m ThreadID, isInitialGen bool) (schedCtx, bool, error) {
	...
	case v2.EvProcStatus:
		...
		oldState := v2ProcStatus2ProcState[status]
		...
		// Bind the proc to the new context, since it's running.
	...
	case v2.EvProcStart:
		...
		if !ok || state.status != v2.ProcIdle || state.seq+1 != seq || curCtx.P != NoProc {
	...
	case v2.EvProcStop:
		...
		if state.status != v2.ProcRunning && state.status != v2.ProcSyscall {
			return curCtx, false, fmt.Errorf("%v event for proc that's not %v or %v", typ, v2.ProcRunning, v2.ProcSyscall)
		}
	...
	case v2.EvProcSteal:
		...
		seq := ev.args[1]
		state, ok := o.pStates[pid]
		if !ok || state.status != v2.ProcSyscall || state.seq+1 != seq {
			// We can't make an inference as to whether this is bad. We could just be seeing
			// a ProcStart on a different M before the proc's state was emitted, or before we
			// got to the right point in the trace.
			//
		...
		//
		// This is safe because we know the P was in a syscall, so the
		// M must still be in a syscall as well. There's going to be
		// some ProcStart on that M, and it won't be able to proceed
		// unless we clear the P on the M.
		mState.p = NoProc
		return curCtx, true, nil
	...
	case v2.EvGoStatus:
		...
		oldState := v2GoStatus2GoState[status]
		if s, ok := o.gStates[gid]; ok {
			if s.status == v2.GoBad {
				oldState = GoNotExist
				s.status = status
			} else if s.status != status {
				return curCtx, false, fmt.Errorf("inconsistent go status: old %v vs. new %v", s.status, status)
			}
			s.seq = 0 // Reset seq.
		} else if isInitialGen {
			// Set the state.
	...
	case v2.EvGoSyscallEnd:
		...
		if pState.status != v2.ProcSyscall {
			return curCtx, false, fmt.Errorf("expected proc %d in state %v, but got %v instead", curCtx.P, v2.ProcSyscall, pState.status)
		}
		pState.status = v2.ProcRunning
		return curCtx, true, nil
	...
	case v2.EvSTWBegin, v2.EvGCSweepBegin, v2.EvGCMarkAssistBegin:
		...
		desc := stringID(0)
		if typ == v2.EvSTWBegin {
			desc = stringID(ev.args[0])
		}
		if err := o.gStates[curCtx.G].beginRange(rangeType{typ, desc}); err != nil {
			return curCtx, false, err
		}
		return curCtx, true, nil
	case v2.EvGCSweepActive, v2.EvGCMarkAssistActive:
		if err := validateCtx(curCtx, event.UserGoReqs); err != nil {
			return curCtx, false, err
		}
		if err := o.gStates[curCtx.G].activeRange(rangeType{typ, 0}, isInitialGen); err != nil {
			return curCtx, false, err
		}
		return curCtx, true, nil
...
func (s *gState) beginRange(typ rangeType) error {
	if s.hasRange(typ) {
		return fmt.Errorf("discovered event already in-flight for goroutine %v when starting event %v", s.id, typ)
	}
...
func (s *gState) activeRange(typ rangeType, isInitialGen bool) error {
	if isInitialGen {
		if s.hasRange(typ) {
			return fmt.Errorf("goroutine %d has named active range already in first gen: %v", s.id, typ)
		}
		s.inFlight = append(s.inFlight, typ)
	} else if !s.hasRange(typ) {
		return fmt.Errorf("goroutine %d is missing active range: %v", s.id, typ)
	}
	return nil
}

// hasRange returns true if a special time range on the goroutine as in progress.
func (s *gState) hasRange(typ rangeType) bool {
...
func (s *gState) endRange(typ event.Type) (stringID, error) {
	...
	if idx < 0 {
		return 0, fmt.Errorf("tried to end event %v, but not in-flight for goroutine %v", st, s.id)
	}
	// Swap remove.
