...
func (o *ordering) advance(ev *baseEvent, evt *evTable, m ThreadID, isInitialGen bool) (schedCtx, bool, error) {
	...
	case v2.EvProcStatus:
		...
		if s, ok := o.pStates[pid]; ok {
			if s.status == v2.ProcBad {
				...
				s.status = status
			} else if status == v2.ProcRunning && s.status == v2.ProcSyscall {
				// ProcSyscall is not a runtime status; it's used purely for validation.
				// Let it slide if we see ProcRunning appear for a P in ProcSyscall.
	...
	case v2.EvSTWBegin, v2.EvGCMarkAssistBegin:
		if err := validateCtx(curCtx, event.UserGoReqs); err != nil {
			return curCtx, false, err
		}
		desc := stringID(0)
	...
	case v2.EvGCMarkAssistActive:
		...
		return curCtx, true, nil
	case v2.EvSTWEnd, v2.EvGCMarkAssistEnd:
		if err := validateCtx(curCtx, event.UserGoReqs); err != nil {
			return curCtx, false, err
		}
...
func makeRangeType(typ event.Type, desc stringID) rangeType {
	if styp := v2.Specs()[typ].StartEv; styp != v2.EvNone {
		typ = styp
	}
...
func (s *rangeState) beginRange(typ rangeType) error {
	if s.hasRange(typ) {
		return fmt.Errorf("discovered event already in-flight for when starting event %v", typ)
	}
...
func (s *rangeState) activeRange(typ rangeType, isInitialGen bool) error {
	if isInitialGen {
		...
		return fmt.Errorf("resource is missing active range: %v %v", v2.Specs()[typ.typ].Name, s.inFlight)
	}
	return nil
}
...
func (s *rangeState) endRange(typ event.Type) (stringID, error) {
	...
	if idx < 0 {
		return 0, fmt.Errorf("tried to end event %v, but not in-flight", st)
	}
	// Swap remove.
