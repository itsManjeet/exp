...
func (b *batch) isStringsBatch() bool {
	return len(b.data) > 0 && event.Type(b.data[0]) == v2.EvStrings
}

func (b *batch) isStacksBatch() bool {
	return len(b.data) > 0 && event.Type(b.data[0]) == v2.EvStacks
}

func (b *batch) isCPUSamplesBatch() bool {
	return len(b.data) > 0 && event.Type(b.data[0]) == v2.EvCPUSamples
}

func (b *batch) isFreqBatch() bool {
	return len(b.data) > 0 && event.Type(b.data[0]) == v2.EvFrequency
}

const maxBatchSize = 64 << 10
...
func readBatch(r *bufio.Reader) (batch, uint64, error) {
	...
	if event.Type(b) != v2.EvEventBatch {
		return batch{}, 0, fmt.Errorf("expected batch event (%d), got %d", v2.EvEventBatch, b)
	}
	...
	if size > maxBatchSize {
		return batch{}, gen, fmt.Errorf("invalid batch size %d, maximum is %d", size, maxBatchSize)
	}
