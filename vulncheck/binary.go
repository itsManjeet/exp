...
func Binary(exe io.ReaderAt, cfg *Config) (*Result, error) {
	...
	modVulns, err := fetchVulnerabilities(cfg.Client, modules(mods))
	if err != nil {
		return nil, err
	}
...
func modules(mods []*packages.Module) []*Module {
	vmods := make([]*Module, len(mods))
	// TODO: should we share unique modules? Not needed now
	// as module info is not returned by Binary.
