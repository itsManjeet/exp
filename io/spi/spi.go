...
// RegisterDriver registers a driver. Open function uses the registered
...
// d returns the driver for the specified driver name.
// If empty string is given, it looks up for devfs driver.
func d(name string) (driver.Driver, error) {
	driversMu.Lock()
	defer driversMu.Unlock()
...
// Transfer performs a duplex transmission to write to the SPI device and read
// len(rx) numbers of bytes.
// It is user's responsibility to not to mutate the tx and rx until
// this call returns.
...
func Open(driverName string, bus, cs int, mode Mode, maxSpeedHz int) (*Device, error) {
	spidriver, err := d(driverName)
	if err != nil {
		return nil, err
	}

	conn, err := spidriver(bus, cs)
	if err != nil {
		return nil, err
	}
