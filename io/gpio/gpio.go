...
const (
	// Low represents low vallues.
	Low = 0

	// High represents high values.
	High = 1
)
...
// Given High, the active value will be high. Given Low, the active value
// will be low.
func (d *Device) SetActive(pin int, v int) error {
	return d.conn.SetActiveLow(pin, v == Low)
}
