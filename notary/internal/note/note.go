...
//
// A note is text signed by one or more verifier keys.
// The text should be ignored unless the note is signed by
// a trusted verifier key and the signature has been verified.
//
// A verifier key is identified by a name, typically the "host[/path]"
...
// A Verifier verifies messages signed with a specific verifier key.
...
// A Signer signs messages using a specific verifier key.
...
// An UnknownVerifierError indicates that the given verifier key is not known.
...
func (e *UnknownVerifierError) Error() string {
	return fmt.Sprintf("unknown verifier key %s+%08x", e.Name, e.KeyHash)
}

// An ambiguousVerifierError indicates that the given name and hash
// match multiple verifier keys passed to VerifierList.
...
func (e *ambiguousVerifierError) Error() string {
	return fmt.Sprintf("ambiguous verifier key %s+%08x", e.name, e.hash)
}
...
type Signature struct {
	...
	// for the verifier key that generated the signature.
	Name string
	Hash uint32
...
// An InvalidSignatureError indicates that the given verifier key was known
...
func (e *InvalidSignatureError) Error() string {
	return fmt.Sprintf("invalid signature for verifier key %s+%08x", e.Name, e.Hash)
}
...
// Open opens and parses the message msg, checking signatures from known verifiers.
//
...
// If any signer uses the same verifier key as an existing signature,
