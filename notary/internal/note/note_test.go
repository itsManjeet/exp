...
func TestVerifierList(t *testing.T) {
	...
	if v != nil || err == nil || err.Error() != "unknown verifier key PeterNeumann+c74f20a4" {
		t.Fatalf("list.Verifier(peter bad hash) = %v, %v, want nil, unknown verifier key error", v, err)
	}

	v, err = list.Verifier("PeterNeuman", 0xc74f20a3)
	if v != nil || err == nil || err.Error() != "unknown verifier key PeterNeuman+c74f20a3" {
		t.Fatalf("list.Verifier(peter bad name) = %v, %v, want nil, unknown verifier key error", v, err)
	}
	v, err = list.Verifier("EnochRoot", 0xaf0cfe78)
	if v != nil || err == nil || err.Error() != "ambiguous verifier key EnochRoot+af0cfe78" {
		t.Fatalf("list.Verifier(enoch) = %v, %v, want nil, ambiguous verifier key error", v, err)
	}
}
...
`
	...
	if err == nil || err.Error() != "ambiguous verifier key EnochRoot+af0cfe78" {
		t.Fatalf("Open with duplicated verifier: err=%v, want ambiguous verifier key", err)
	}
	...
	if n != nil || err == nil || err.Error() != "invalid signature for verifier key PeterNeumann+c74f20a3" {
