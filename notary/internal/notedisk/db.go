...
func open(name string, create bool) (_ *DB, err error) {
	var db *DB // not overwritten by 'return nil, err'
	defer func() {
...
func (db *DB) ReadHashes(indexes []int64) ([]tlog.Hash, error) {
	db.reload()
...
// ReadHash reads a single hash from the database.
func (db *DB) ReadHash(level int, n int64) (tlog.Hash, error) {
	db.reload()
	return db.readHash(tlog.StoredHashIndex(level, n))
}

func (db *DB) readHash(index int64) (tlog.Hash, error) {
	var h tlog.Hash
	_, err := db.hash.ReadAt(h[:], int64(hashSize)*index)
	if err != nil {
		if err == io.EOF {
			err = fmt.Errorf("no such hash %d", index)
		}
