...
func (c *Conn) initWork() {
	...
	verifier, err := note.NewVerifier(string(bytes.TrimSpace(vkey)))
	if err != nil {
		c.initErr = err
		return
	}
...
Globs:
	for _, glob := range c.noverify {
		...
		pos := 0
		for n := strings.Count(glob, "/") + 1; n > 0; n-- {
			i := strings.Index(target[pos:], "/")
			if i < 0 {
				if n > 1 {
					// Too short.
					continue Globs
				}
				// Pretend to find final slash at end of target.
				i = len(target[pos:])
			}
			pos += i + 1
		}
		matched, _ := path.Match(glob, target[:pos-1])
		if matched {
			return true
		}
	}
...
func (c *Conn) Lookup(path, vers string) (lines []string, err error) {
	...
	result := c.record.Do(file, func() interface{} {
		...
		if err != nil {
			data, err = c.client.GetURL("https://" + file)
			if err != nil {
...
func (c *Conn) tileURL(tile tlog.Tile) string {
	return "https://" + c.tileCacheKey(tile)
}
