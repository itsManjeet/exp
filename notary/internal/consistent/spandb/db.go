...
// Package spandb implements a consistent.DB using Google Cloud Spanner.
package spandb
...
func (db *DB) CreateTables(ctx context.Context, tables []*consistent.Table) error {
	if db.admin == nil {
		adminClient, err := admin.NewDatabaseAdminClient(ctx)
		if err != nil {
			return err
...
func (db *DB) ReadOnly(ctx context.Context, f func(context.Context, consistent.Transaction) error) error {
	tx := db.client.ReadOnlyTransaction()
	return f(ctx, &spannerTx{r: tx})
}

// ReadWrite executes f in a read-write transaction.
func (db *DB) ReadWrite(ctx context.Context, f func(context.Context, consistent.Transaction) error) error {
...
func (tx *spannerTx) Read(ctx context.Context, table string, keys consistent.Keys, columns []string) consistent.Rows {
	return &spannerRows{tx.r.Read(ctx, table, toKeySet(keys), columns)}
}

// A spannerRows is an implementation of consistent.Rows backed by a Spanner row iterator.
type spannerRows struct {
	rows *spanner.RowIterator
}

func (r *spannerRows) Do(f func(consistent.Row) error) error {
...
func (tx *spannerTx) ReadRow(ctx context.Context, table string, key consistent.Key, columns []string) (consistent.Row, error) {
...
func (tx *spannerTx) BufferWrite(writes []consistent.Mutation) error {
	if tx.w == nil {
...
// consistentError maps err to an appropriate error for the consistent.DB interfaces.
func consistentError(err error) error {
	// Detect row not exist and turn it into consistent.RowNotExistError.
	if err != nil && spanner.ErrCode(err) == codes.NotFound {
		err = &consistent.RowNotExistError{Err: err}
	}
	return err
}

// toKey maps a consistent.Key to a spanner.Key.
// (The representations are the same.)
func toKey(key consistent.Key) spanner.Key {
	return []interface{}(key)
}

// toKeySet maps a consistent.Keys to a spanner.KeySet.
func toKeySet(keys consistent.Keys) spanner.KeySet {
	if keys.All {
