...
func StoredHashesForRecordHash(n int64, h Hash, r HashReader) ([]Hash, error) {
	...
	// Build indexes of needed hashes.
	var indexes []int64
	level := 0
	for i := n; i&1 == 1; {
		i >>= 1
		indexes = append(indexes, StoredHashIndex(level, 2*i))
		level++
	}
	...
	for i := n; i&1 == 1; {
		i >>= 1
		hashes = append(hashes, hashNode(old[level], hashes[len(hashes)-1]))
		level++
	}
	return hashes, nil
}
...
// The tree of size n is defined to have an all-zero Hash.
