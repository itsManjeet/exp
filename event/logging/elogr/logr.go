...
func addLabels(b event.Builder, keysAndValues []interface{}) event.Builder {
	for i := 0; i < len(keysAndValues); i += 2 {
		b = b.With(newLabel(keysAndValues[i], keysAndValues[i+1]))
	}
	return b
}
...
func (l *logger) log(msg string, b event.Builder, keysAndValues []interface{}) {
	b = b.With(internal.LevelKey.Of(l.verbosity)) // TODO: Convert verbosity to level.
	b = b.With(internal.NameKey.Of(l.name))
	addLabels(b, keysAndValues).Log(msg)
}
...
func (l *logger) WithValues(keysAndValues ...interface{}) logr.Logger {
	l2 := *l
	l2.builder = addLabels(l2.builder.Clone(), keysAndValues)
	return &l2
}
