...
func (c *core) With(fields []zapcore.Field) zapcore.Core {
	c2 := *c
	c2.builder = addLabels(c2.builder.Clone(), fields)
	return &c2
}
...
func (c *core) Write(e zapcore.Entry, fs []zapcore.Field) error {
	...
	if e.Stack != "" {
		b = b.With(keys.String("stack").Of(e.Stack))
	}
	if e.Caller.Defined {
		b = b.With(keys.String("caller").Of(e.Caller.String()))
	}
	addLabels(b, fs).Log(e.Message)
	return nil
}
...
func addLabels(b event.Builder, fields []zap.Field) event.Builder {
	for i := 0; i < len(fields); i++ {
		b = b.With(newLabel(fields[i]))
	}
	return b
}
