...
// This file is a modified copy of golang.org/x/tools/internal/jsonrpc2/conn.go.
...
type conn struct {
	...
	err                  atomic.Value
	calls                *event.Counter
	bytesSent, bytesRecv *event.IntDistribution
}
...
func NewConn(s Stream) Conn {
	conn := &conn{
		...
		calls:     event.NewCounter("calls"),
		bytesSent: event.NewIntDistribution("bytesSent"),
		bytesRecv: event.NewIntDistribution("bytesReceived"),
	}
	return conn
}
...
func (c *conn) Notify(ctx context.Context, method string, params interface{}) (err error) {
	...
	ctx, done := event.To(ctx).
		With(Method.Of(method)).
		With(RPCDirection.Of("outbound")).
		Start(method)
	defer func() {
		c.calls.To(ctx).With(Method.Of(method)).With(Error.Of(err)).Record(1)
		done()
	}()

	n, err := c.write(ctx, notify)
	c.bytesSent.To(ctx).With(Method.Of(method)).Record(n)
	return err
}
...
func (c *conn) Call(ctx context.Context, method string, params, result interface{}) (_ ID, err error) {
	...
	ctx, done := event.To(ctx).
		With(Method.Of(method)).
		With(RPCDirection.Of("outbound")).
		With(RPCID.Of(fmt.Sprintf("%q", id))).
		Start(method)
	defer func() {
		c.calls.To(ctx).With(Method.Of(method)).With(Error.Of(err)).Record(1)
		done()
	}()
	c.calls.To(ctx).With(Method.Of(method)).Record(1)
...
func (c *conn) replier(req Request, spanDone func()) Replier {
	return func(ctx context.Context, result interface{}, err error) error {
		defer func() {
			c.calls.To(ctx).With(Method.Of(req.Method())).With(Error.Of(err)).Record(1)
			spanDone()
		}()
		call, ok := req.(*Call)
		if !ok {
		...
		c.bytesSent.To(ctx).With(Method.Of(req.Method())).Record(n)
		if err != nil {
...
func (c *conn) run(ctx context.Context, handler Handler) {
	...
	for {
		...
		case Request:
			...
			reqCtx, spanDone := event.To(ctx).WithAll(labels...).Start(msg.Method())
			c.bytesRecv.To(ctx).With(Method.Of(msg.Method())).Record(n)
			if err := handler(reqCtx, c.replier(msg, spanDone), msg); err != nil {
				// delivery failed, not much we can do
				fmt.Println("jsonrpc2 message delivery failed", err)
			}
		case *Response:
