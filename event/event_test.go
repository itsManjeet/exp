...
`}, {
		...
		expect: `time=2020-03-05T14:27:48 l1=1 metricValue=2 metric=Gauge("golang.org/x/exp/event/gauge")`,
	}, {
		name: "metric 2",
		events: func(ctx context.Context) {
			event.To(ctx).With(l1).With(l2).Metric(latency, event.DurationOf(3*time.Second))
		},
		expect: `time=2020-03-05T14:27:48 l1=1 l2=2 metricValue=3s metric=Distribution("golang.org/x/exp/event/latency")`,
	}, {
		name:   "annotate",
