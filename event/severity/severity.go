...
func (l Level) Log(ctx context.Context, msg string) {
	event.To(ctx).As(event.LogKind).
		Label(l.Label()).
		Message(msg).
		Send()
}

func (l Level) LogB(ctx context.Context, msg string) event.Builder {
	return event.To(ctx).As(event.LogKind).
		Label(l.Label()).
		Message(msg)
}
...
func (l Level) Logf(ctx context.Context, msg string, args ...interface{}) {
	...
	b.Label(event.Label{Name: Key, Value: event.ValueOf(l)}).
