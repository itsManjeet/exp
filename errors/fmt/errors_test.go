...
func TestErrorf(t *testing.T) {
	...
	testCases := []struct {
		...
		chain(`nounwrap: simple+TestErrorf+go:xxx`),
	}, {
		fmt.Errorf("nounwrap: %v", "simple"),
		chain(`nounwrap: simple+TestErrorf+go:xxx`),
	}, {
		fmt.Errorf("%s failed: %v", "foo", chained),
		chain("wraps:foo failed+TestErrorf+go:xxx",
			"chained/somefile.go:xxx"),
	}, {
		fmt.Errorf("wraps: %s", chained),
		chain("wraps:wraps+TestErrorf+go:xxx", "chained/somefile.go:xxx"),
	}, {
		fmt.Errorf("wrapv: %v", chained),
		chain("wraps:wrapv+TestErrorf+go:xxx", "chained/somefile.go:xxx"),
	}, {
		fmt.Errorf("not wrapped: %+v", chained),
		chain("not wrapped: chainedsomefile.go:123+TestErrorf+go:xxx"),
	}}
