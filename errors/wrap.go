...
//
// An error is considered to be equal to a target if it is equal to that target
// or if it implements an Is method such that Is(target) returns true.
...
// can be represented as a type if it is of the same type, or if it has an As
// method such that As(target) returns true. As will panic if target is nil.
