...
import (
	...
	"golang.org/x/exp/sumdb/internal/googlecloud/spanner"
...
const spannerDB = "projects/rsc-goog/instances/rsc-test/databases/test_sumdb"

func main() {
	flag.Parse()
	handler := &sumweb.Handler{Server: noteServer{}}
...
func (noteServer) Lookup(ctx context.Context, key string) (int64, error) {
	// TODO: Store @ keys.
	key = strings.Replace(key, "@", " ", 1)
	return getDB(ctx).Lookup(ctx, key)
}
...
func createDB(ctx context.Context, w io.Writer, args []string) {
	sdb, err := spanner.CreateStorage(ctx, spannerDB)
	if err != nil {
		fmt.Fprintf(w, "?%v\n", err)
		return
	}
	_, err = database.Create(ctx, sdb)
	if err != nil {
		fmt.Fprintf(w, "?%v\n", err)
		return
	}
...
func deleteDB(ctx context.Context, w io.Writer, args []string) {
	err := spanner.DeleteTestStorage(ctx, spannerDB)
	if err != nil {
		fmt.Fprintf(w, "?%v\n", err)
		return
	}
...
func getDB(ctx context.Context) *database.DB {
	dbcache.Lock()
	...
	if *memFlag {
		db, err := database.Create(ctx, new(tkvtest.Mem))
		if err != nil {
			panic(err)
		}
		dbcache.db = db
	...
	sdb, err := spanner.OpenStorage(ctx, spannerDB)
	if err != nil {
		panic(err)
	}
	db, err := database.Open(ctx, sdb)
	if err != nil {
		panic(err)
	}
	dbcache.db = db
...
func insert(ctx context.Context, w io.Writer, args []string) {
	...
	for len(recs) > 0 {
		...
		p := *pFlag
		for i := 0; i < p; i++ {
			go func(recs []database.NewRecord) {
				c <- db.Add(ctx, recs)
...
<h1>Go Checksum Database Shell</h1>
