...
func Create(ctx context.Context, store tkv.Storage) (*DB, error) {
	// Only initialization is to record tree size 0.
	err := store.ReadWrite(ctx, func(ctx context.Context, tx tkv.Transaction) error {
		if val, err := tx.ReadValue(ctx, "size"); err != nil {
			return err
		} else if val != "" {
			return fmt.Errorf("database already populated")
		}
		return tx.BufferWrites([]tkv.Write{{Key: "size", Value: encodeInt(0)}})
	})
	if err != nil {
		return nil, err
	}
...
func Open(ctx context.Context, store tkv.Storage) (*DB, error) {
	// Check that the database is initialized,
	// by reading the tree size.
	db := &DB{store: store}
	err := store.ReadOnly(ctx, func(ctx context.Context, tx tkv.Transaction) error {
...
func (db *DB) NumRecords(ctx context.Context) (int64, error) {
	var size int64
	err := db.store.ReadOnly(ctx, func(ctx context.Context, tx tkv.Transaction) error {
		var err error
...
func (db *DB) writeTreeSize(ctx context.Context, tx tkv.Transaction, size int64) error {
	return tx.BufferWrites([]tkv.Write{{Key: "size", Value: encodeInt(size)}})
}

// readTreeSize reads and returns the current tree size within a transaction.
func (db *DB) readTreeSize(ctx context.Context, tx tkv.Transaction) (int64, error) {
...
func (db *DB) readHashesInTx(ctx context.Context, tx tkv.Transaction, need hashMapLogger) (hashMap, error) {
	var keys []string
	for id := range need {
...
func (db *DB) SetConfig(ctx context.Context, key, value string) error {
	return db.store.ReadWrite(ctx, func(ctx context.Context, tx tkv.Transaction) error {
		return tx.BufferWrites([]tkv.Write{{Key: "config:" + key, Value: value}})
	})
}
...
func (db *DB) Add(ctx context.Context, records []NewRecord) error {
	...
	err := db.store.ReadWrite(ctx, func(ctx context.Context, tx tkv.Transaction) error {
		...
		for _, rec := range recs {
			...
			writes = append(writes,
				tkv.Write{Key: "record:" + encodeInt(rec.ID), Value: string(rec.Content)},
				tkv.Write{Key: "module:" + rec.Key, Value: encodeInt(rec.ID) + encodeHash(rec.rhash)},
			)
			...
			for _, h := range toStore {
				writes = append(writes, tkv.Write{Key: "hash:" + encodeInt(storageID), Value: encodeHash(h)})
				hashes[storageID] = h
				storageID++
			}
