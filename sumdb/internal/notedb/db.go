...
// Package notedb implements relational database-backed storage
// for the Go checksum database.
//
// This package is part of a DRAFT of what the Go checksum database will look like.
...
var schema = []*database.Table{
	...
	},
	{
		...
		// It holds configuration settings (see Config and SetConfig)
	...
	},
	{
		...
		// to hash(record data) (recordhash) and the record's level 0 id in the checksum database log.
...
// Create initializes a new checksum database given an empty database.DB.
...
// Open opens an existing checksum database stored in a database.DB.
...
func (db *DB) ReadRecords(ctx context.Context, id, n int64) ([][]byte, error) {
	var list [][]byte
	err := db.database.ReadOnly(ctx, func(ctx context.Context, tx database.Transaction) error {
		// TODO(rsc): Batch this into two large reads.
		end := id + n
		for ; id < end; id++ {
			var data []byte
