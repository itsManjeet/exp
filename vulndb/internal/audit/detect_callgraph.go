...
//
// Findings for each vulnerability are sorted by estimated
// usefulness to the user.
//
// Panics if packages in pkgs do not belong to the same program.
func VulnerableSymbols(pkgs []*ssa.Package, env Env) Results {
	results := Results{
	...
	for queue.Len() > 0 {
		...
		calls := funcVulnsAndCalls(v, symVulns, env, &results, callGraph)
...
func funcVulnsAndCalls(chain *callChain, symVulns symVulnerabilities, env Env, results *Results, callGraph *callgraph.Graph) []*callChain {
	var calls []*callChain
	for _, b := range chain.f.Blocks {
		for _, instr := range b.Instrs {
			// First collect all findings for globals except callees in function call statements.
			globalFindings(globalUses(instr), chain, symVulns, env, results)
			...
			for _, callee := range siteCallees(site, callGraph) {
				...
				callFinding(c, symVulns, env, results)
			}
		}
	}
	return calls
...
func globalFindings(globalUses []*ssa.Value, chain *callChain, symVulns symVulnerabilities, env Env, results *Results) {
...
func callFinding(chain *callChain, symVulns symVulnerabilities, env Env, results *Results) {
	if underRelatedVuln(chain, symVulns, env) {
		return
	}
