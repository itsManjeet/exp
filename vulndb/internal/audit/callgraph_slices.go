...
type cgSlice map[ssa.CallInstruction]bool

func vulnSlice(funcs map[*ssa.Function]bool, entries []*ssa.Function, cg *callgraph.Graph, modVulns ModuleVulnerabilities) cgSlice {
	...
	return computeSlice(sinks, fcs, cg)
...
func computeSlice(sinks map[*ssa.Function]bool, fslice map[ssa.CallInstruction]bool, cg *callgraph.Graph) cgSlice {
...
func sliceSizes(slice cgSlice, entries []*ssa.Function) (int, int, int) {
...
func forwardCallSites(sources []*ssa.Function, cg *callgraph.Graph) cgSlice {
	m := make(map[*ssa.Function]bool)
	callSites := make(cgSlice)
...
func forwardCall(f *ssa.Function, cg *callgraph.Graph, seen map[*ssa.Function]bool, callSites cgSlice) {
	if _, ok := seen[f]; ok {
		return
	}
	seen[f] = true
...
func backwardCallSites(sinks map[*ssa.Function]bool, cg *callgraph.Graph) cgSlice {
	callSites := make(cgSlice)
