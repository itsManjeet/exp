...
func (d *Device) SetMode(mode int) error {
	m := uint8(mode)
	if err := d.ioctl(requstCode(write, magic, 1, 1), uintptr(unsafe.Pointer(&m))); err != nil {
...
func (d *Device) SetSpeed(speedHz int) error {
	s := uint32(speedHz)
	if err := d.ioctl(requstCode(write, magic, 4, 4), uintptr(unsafe.Pointer(&s))); err != nil {
...
func (d *Device) SetBitsPerWord(bits int) error {
	b := uint8(bits)
	if err := d.ioctl(requstCode(write, magic, 3, 1), uintptr(unsafe.Pointer(&b))); err != nil {
...
// requstCode returns the device specific request code for the specified direction,
// type, number and size to be used in the ioctl call.
func requstCode(dir, typ, nr, size uintptr) uintptr {
