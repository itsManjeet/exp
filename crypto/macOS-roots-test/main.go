...
func main() {
	...
	t0 := time.Now()
	sysRoots_, err := loadSystemRoots() // actual system roots
	sysRootsDuration := time.Since(t0)

	if err != nil {
		log.Fatalf("failed to read system roots (cgo): %v", err)
	}
	sysRoots := (*CertPool)(unsafe.Pointer(sysRoots_))

	t1 := time.Now()
	execRoots_, err := execSecurityRoots() // non-cgo roots
	execSysRootsDuration := time.Since(t1)

	if err != nil {
		log.Fatalf("failed to read system roots (nocgo): %v", err)
	}
	execRoots := (*CertPool)(unsafe.Pointer(execRoots_))
	...
	for _, c := range execRoots.certs {
		if _, ok := sysPool[string(c.Raw)]; ok {
			...
			if _, err := c.Verify(x509.VerifyOptions{
				Roots:         sysRoots_,
				Intermediates: execRoots_, // the intermediates for EAP certs are stored in the keychain
