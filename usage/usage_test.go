		...
		t.Run(test.name, func(t *testing.T) {
			...
			for _, v := range test.values {
				t.Run(strings.Join(v.args, "‿"), func(t *testing.T) {
					options := &Options{}
					fields, _ := usage.FieldsOf(options)
					err := usage.Process(test.help, fields, v.args)
					switch {
					case v.err == "" && err != nil:
...
func TestCompileErrors(t *testing.T) {
	for _, test := range []struct {
	...
	}{
		{
			...
			expect: `^help:2:28: expect "]" at "\\n# not`,
		}, {
			name:   `invalid line default`,
			help:   "usage:\n  -flag  comment \n[default: \n# not closed",
			expect: `^help:3:11: expect "]" at "\\n# not`,
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			help := []usage.Page{{Name: "help", Content: test.help}}
			// now run all the tests with args
			err := usage.Process(help, nil, nil)
			if err == nil {
