...
// Process compiles and uses the help text declaration to build a grammar.
// It then matches that grammar against the args.
// It attempts to apply the results of that match to the fields. It will do this
// even if the match failed, so that you can see the partial results. This can
// help with producing better error messages.
func Process(help []Page, fields *Fields, args []string) error {
	...
	// find fields for each expression in the grammar
	// we do this to verify the grammar, even though we might not need them all
	bindings, err := bind(g, fields)
	if err != nil {
		return err
	}
	// and apply the results to those bindings
	err = bindings.apply(results)
...
// FindPage returns a page with the given name.
// The names are compared using EqualFold. This is mainly used to support
// a help command that can print specific pages.
func FindPage(pages []Page, name string) Page {
...
// printExpression is a debugging helper, and not exposed through any public API
func printExpression(expr expression) {
	...
	case *section:
		fmt.Print(expr.title)
		fmt.Print(":")
		printExpression(expr.root)
		fmt.Print("\n")
	case sequence:
		fmt.Print("(")
		for i, e := range expr {
