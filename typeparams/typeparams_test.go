...
func writeTuple(buf *bytes.Buffer, t *types.Tuple) {
	...
	// Cheap trick: the API at Go 1.18 uses aliases for types in go/types. These
	// types are _actually_ in the go/types package, and therefore would be
	// formatted as e.g. *types.TypeParam, which would not match
	// *typeparams.TypeParam -- go/types does not track aliases. As we use the
	// same name for all aliases, we can make the formatted signatures match by
	// dropping the package qualifier.
