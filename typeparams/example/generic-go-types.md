...
the new language constructs introduced for generic Go.
...
abstractly the set of types that implement an interface.
...
the numeric type may only be satisfied by types whose underlying is `int` or
`float64`.
...
`types.Union` type, which flattens the expression into a list of terms. We can
also investigate two new methods of interface: `types.Interface.IsComparable`,
which reports whether the type set of an interface is comparable, and
`types.Interface.IsMethodSet`, which reports whether an interface is
expressable using methods alone.
...
ordinary Go syntax, whose type-set consists of all comparable concrete types.
...
interface type was implicit. This is does not affect the behavior of the
...
In this example, the type set of `D` simplifies to `~string|int`, but the
current `go/types` APIs do not expose this information. This will likely be
added to `go/types` in future versions of Go, but in the meantime we can use
the `typeparams.NormalTerms` helper:
...
via type inference. This section described how to find and understand
instantiated types.
...
type arguments that were used to instantiate the instance, and the resulting
type. Notably, it does not include the _generic_ type that was instantiated,
because this type can be found using `types.Info.Uses[id].Type()` (where `id`
...
`go/types` also provides an API creating type instances: `types.Instantiate`.
This function accepts a generic type and type arguments, and returns an
instantiated type (or an error). The resulting instance may be a newly
constructed type, or a pre-existing instance with the same type identity:
`types.Instantiate` accepts a `types.Context` as its first argument, which
records instances for later reuse.
...
constraint. If they don't, the resulting error will wrap a new `ArgumentError`
...
a generic type: generic types cannot be used without instantation, and so can't
be the type of the receiver variable. Instead, the receiver base type is an
...
Predicates on generic types are not defined by the spec. For example, it is not
clear what `types.AssignableTo` means with respect to a generic type.
...
`*types.Signature` types are treated differently. Similar to ordinary signature
...
between generic types. In general, in order to phrase these questions we need
more information: how does one relate the type parameters of `Pair` to the type
...
we can determing if, for all type arguments `[X, Y]` that are valid for `Pair`,
