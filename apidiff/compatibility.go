...
var (
	size32 = types.StdSizes{WordSize: 4, MaxAlign: 1}
	size64 = types.StdSizes{WordSize: 8, MaxAlign: 1}
)

// isCompatibleBasic reports whether old and new are compatible types.
func isCompatibleBasic(old, new *types.Basic) bool {
	// Old and new must be of the same ilk (unsigned int, signed int, float or complex)
	if old.Info() != new.Info() {
		return false
	}
	// They must be numeric.
	if old.Info()&types.IsNumeric == 0 {
		return false
	}
	// Neither can be uintptr.
	if old.Kind() == types.Uintptr || new.Kind() == types.Uintptr {
		return false
	}
	// New must be at least as large as old on both 32-bit and 64-bit machines.
	return size32.Sizeof(old) <= size32.Sizeof(new) && size64.Sizeof(old) <= size64.Sizeof(new)
}
