...
func (d *differ) checkCompatibleChan(otn *types.TypeName, old, new *types.Chan) {
	d.checkEquivalent(otn, ", element type", old.Elem(), new.Elem())
	if old.Dir() != new.Dir() {
...
func (d *differ) checkCompatibleObjectSets(obj types.Object, old, new map[string]types.Object) {
	for name, oldo := range old {
		...
		if newo == nil {
			...
			d.checkEquivalent(obj, name, oldo.Type(), newo.Type())
		}
	}
	for name := range new {
		if old[name] == nil {
...
func (d *differ) checkCompatibleNamed(otn *types.TypeName, old, new *types.Named) {
	if !d.corresponds(old, new) {
		d.incompatible(otn, "", "does not correspond to %s", new.Obj().Name())
	}
	...
	d.checkNamedMethodSet(otn, old, new, false)
	d.checkNamedMethodSet(otn, old, new, true)
}

func (d *differ) checkNamedMethodSet(otn *types.TypeName, old, new *types.Named, pointer bool) {
	var (
		oldt types.Type = old
		newt types.Type = new
	)
	if pointer {
		oldt = types.NewPointer(oldt)
		newt = types.NewPointer(newt)
	}
	d.checkMethodSet(otn, oldt, newt, true)
}
...
func (d *differ) checkMethodSet(otn *types.TypeName, oldt, newt types.Type, addcompat bool) {
	...
	for name, oldobj := range oldms {
		...
		if newobj == nil {
			...
			d.checkEquivalent(obj, "", oldobj.Type(), newobj.Type())
		}
	}
