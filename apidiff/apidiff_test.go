...
func TestChanges(t *testing.T) {
	...
	defer os.RemoveAll("/tmp/new")
	sort.Strings(wanti)
	sort.Strings(wantc)

	oldpkg, err := load("/tmp/old")
	if err != nil {
		t.Fatal(err)
	}
	newpkg, err := load("/tmp/new")
	if err != nil {
		t.Fatal(err)
	}
...
func splitIntoPackages(t *testing.T) (incompatibles, compatibles []string) {
	...
	if err := os.MkdirAll("/tmp/old", 0700); err != nil {
		t.Fatal(err)
	}
	if err := os.Mkdir("/tmp/new", 0700); err != nil && !os.IsExist(err) {
		t.Fatal(err)
	}

	oldf, err := os.Create("/tmp/old/old.go")
	if err != nil {
		t.Fatal(err)
	}
	newf, err := os.Create("/tmp/new/new.go")
	if err != nil {
		t.Fatal(err)
	}
