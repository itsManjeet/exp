...
func objectString(obj types.Object, rootPackagePath string) string {
	...
	if thisPackagePath == rootPackagePath {
		...
		// obj is outside the diff operation root! - display full path. This should
		// generally not be possible to occur, but it ensures clarity when it does,
		// due to code bugs such as https://github.com/golang/go/issues/61385.
