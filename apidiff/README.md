...
Here we list five ways in which code may fail to compile after a change. This
...
defined types have the same name instead. But that doesn't work either, for two
reasons. First, type aliases can equate two types with different names:
...
```
Although this is unlikely, changes to constant values are rare, so we feel it is
...
and receive on them. (Channel values can also be used as map keys, but that
cannot cause a compile-time error.)
...
"splits" an identical underlying type into two, breaking conversions:
...
The above definition states that the only types that can differ compatibly are
