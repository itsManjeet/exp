...
because it is an error to select a field that appears more than once with the
same name at the same depth.
...
We say that two defined types _correspond_ if they are used in the same places
in the old and new APIs.
...
above demonstrates that we can't weaken this condition to name equality.
...
Like any tool that uses go/types, the checker has to pick a configuration
when loading packages. Changes to GOOS and GARCH, as well as the presence of
build tags, may affect the configuration.
...
> For every exposed type that implements an exposed interface in the old package,
...
whenever two identical types exist in the old or new package. In this example, a
compatible change to a channel type makes it identical to another type, breaking
type switches:
...
// new
type B =   chan int // unchanged
type C =   chan int // compatible change
...
Here, the change "splits" an identical type into two, breaking assignments:
...
This tool doesn't check for this sort of problem.
