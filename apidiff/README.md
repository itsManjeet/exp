...
The `apidiff` tool in this directory determines whether two
...
Although `pkt.t` can't be referred to directly by a client, values of type `t`
can be used. For example, client code can call the `t.M` method via the exported
variable `V`.
...
2. Otherwise (if it is not the case that one type is from the old package and
   the other from the new), the types are equivalent if and only if they have
   the same name and package.
...
// client
var f func(io.Writer) = func(pkg.I) {} // fails in new: different types in assignment
```
...
>same name if their types (signatures) are equivalent.
...
> A new exported typename is compatible with an old one if their names are the
> same and their types are equivalent.
...
that the struct literals have to be equivalent, only that the defined types
denoted by `T` must be equivalent (that is, correspond).
...
If an interface has an unexported method, the only way a client can implement it
is to embed it in a struct, so it is safe to add methods.
