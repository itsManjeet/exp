...
identical, and by assumption the old and new types underly defined types (see
"Compatibility, Types and Names," below), there is no way for client code to
write an arithmetic expression that compiles with operands of the old type but
not the new.
...
constant is a compile-time error. That is why the only compatible changes are to
