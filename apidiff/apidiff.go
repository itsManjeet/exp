...
func (d *differ) checkPackage() {
	...
	for otn1, nt1 := range d.correspondMap {
		for otn2, nt2 := range d.correspondMap {
			...
			oiface, ok := otn2.Type().Underlying().(*types.Interface)
			if ok && types.Implements(otn1.Type(), oiface) {
				// Here, old exposed type otn1 implements old interface oiface.
				// So the corresponding new types must have the same relationship.
				if niface, ok := nt2.Underlying().(*types.Interface); ok {
					if !types.Implements(nt1, niface) {
...
func (d *differ) constChanges(old, new *types.Const) {
	...
	// QUESTION: are there constant values that are not comparable?
