...
func (d *differ) constChanges(old, new *types.Const) {
	ot := old.Type()
	nt := new.Type()
	// Check for change to untyped.
	if !isUntyped(ot) && isUntyped(nt) && asUntyped(ot) == nt.(*types.Basic).Info() {
		d.compatible(old, "", "changed from %s to %s", ot, nt)
	} else if !d.equivalent(ot, nt) {
...
// Return the BasicInfo for the untyped form of a basic type.
func asUntyped(typ types.Type) types.BasicInfo {
	// Remove unsigned, add untyped.
	return typ.Underlying().(*types.Basic).Info()&^types.IsUnsigned | types.IsUntyped
}
