...
func (d *differ) establishCorrespondence(old *types.Named, new types.Type) bool {
	...
	if newn, ok := new.(*types.Named); ok {
		...
		// Two instantiated named types correspond if their type argument lists correspond.
		// TODO(jba): do we have to check that their origins correspond? We weren't doing that
		// before.
		// Two generic named types correspond if their type parameter lists correspond.
		// Since one or the other of those lists will be empty, it doesn't hurt to check both.
