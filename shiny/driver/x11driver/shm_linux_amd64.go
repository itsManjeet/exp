...
func shmOpen(size int) (shmid uintptr, addr unsafe.Pointer, err error) {
	...
	if errno0 != 0 {
		return 0, unsafe.Pointer(uintptr(0)), fmt.Errorf("shmget failed: %v", errno0)
	}
	...
	if errno1 != 0 {
		return 0, unsafe.Pointer(uintptr(0)), fmt.Errorf("shmat failed: %v", errno1)
	}
	if errno2 != 0 {
		return 0, unsafe.Pointer(uintptr(0)), fmt.Errorf("shmctl failed: %v", errno2)
	}
