...
func shmOpen(size int) (shmid uintptr, addr unsafe.Pointer, err error) {
	...
	if errno0 != 0 {
		return 0, unsafe.Pointer(uintptr(0)), fmt.Errorf("x11driver: shmget failed: %v", errno0)
	}
	...
	if errno1 != 0 {
		return 0, unsafe.Pointer(uintptr(0)), fmt.Errorf("x11driver: shmat failed: %v", errno1)
	}
	if errno2 != 0 {
		return 0, unsafe.Pointer(uintptr(0)), fmt.Errorf("x11driver: shmctl failed: %v", errno2)
	}
...
func shmClose(p unsafe.Pointer) error {
	...
	if errno != 0 {
		return fmt.Errorf("x11driver: shmdt failed: %v", errno)
	}
	return nil
}
