...
func (*screenImpl) NewBuffer(size image.Point) (screen.Buffer, error) {
	...
	// between 386 and amd64, so just using int32 for now.
	const maxBufLen = math.MaxInt32
	if size.X < 0 || size.X > math.MaxInt32 || size.Y < 0 || size.Y > math.MaxInt32 || int64(size.X)*int64(size.Y)*4 > maxBufLen {
