...
func drawLoop(w *windowImpl) {
	...
	for {
		select {
		case <-workAvailable:
			win32.SendMessage(win32.HWND(w.id), msgGLDoWork, 0, 0)
		case <-w.publish:
			fmt.Printf("drawLoop: publish\n")
			win32.SendMessage(win32.HWND(w.id), msgSwapBuf, 0, 0)
		}
	}
}

var (
	msgGLDoWork = win32.AddWindowMsg(glDoWork)
	msgSwapBuf  = win32.AddWindowMsg(swapBuf)
)
...
func createEGLSurface(hwnd win32.HWND, w *windowImpl) error {
	...
	if ret, _, _ := eglMakeCurrent.Call(display, surface, surface, context); ret == 0 {
		return fmt.Errorf("eglMakeCurrent failed: %v", eglErr())
	}
...
func glDoWork(hwnd win32.HWND, uMsg uint32, wParam, lParam uintptr) bool {
	theScreen.mu.Lock()
	w := theScreen.windows[uintptr(hwnd)]
	theScreen.mu.Unlock()
...
func swapBuf(hwnd win32.HWND, uMsg uint32, wParam, lParam uintptr) bool {
	theScreen.mu.Lock()
	w := theScreen.windows[uintptr(hwnd)]
	theScreen.mu.Unlock()
...
loop:
	...
	fmt.Printf("eglSwapBuffers called\n")
	return false
}
