...
type Buffer interface {
	// Close releases the Buffer's resources. The Buffer should not be used
	// after Close is called.
	//
	// Close returns an error to conform to the standard io.Closer interface,
	// but the error will always be nil.
	Close() error
...
type Texture interface {
	// Close releases the Texture's resources. The Texture should not be used
	// after Close is called.
	//
	// Close returns an error to conform to the standard io.Closer interface,
	// but the error will always be nil.
...
// When drawing on a Window, there might not be any visible effect until
// EndPaint is called.
func Scale(dst Drawer, dr image.Rect, src Texture, sr image.Rectangle, op draw.Op, opts *DrawOptions)
