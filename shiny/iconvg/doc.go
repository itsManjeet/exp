...
/*
Package iconvg implements a compact, binary format for vector graphics.

WARNING: THIS FORMAT IS EXPERIMENTAL AND SUBJECT TO INCOMPATIBLE CHANGES.

OVERVIEW

IconVG is a format for simple vector graphics: icons, logos, glyphs and emoji.
...
may appear when scaling up to extreme sizes. Nonetheless, at typical scales,
such differences are not expected to be perceptible to the naked eye.
...
implicit M (absolute moveto). CREG[CSEL-ADJ], either a flat color or a
...
their repeat count N: how often that drawing operation is repeated. The lowest
...
Opcodes 0x00 to 0x1f means N consecutive L ops, for N in [1, 32]. The opcode is
followed by 2*N coordinates, N sets of (x, y).

Opcodes 0x20 to 0x3f are like the previous paragraph, except L (absolute)
becomes l (relative).

Opcodes 0x40 to 0x4f means N consecutive T ops, for N in [1, 16]. The opcode is
followed by 2*N coordinates, N sets of (x, y).

Opcodes 0x50 to 0x5f are like the previous paragraph, except T (absolute)
becomes t (relative).

Opcodes 0x60 to 0x6f means N consecutive Q ops, for N in [1, 16]. The opcode is
followed by 4*N coordinates, N sets of (x1, y1, x, y).

Opcodes 0x70 to 0x7f are like the previous paragraph, except Q (absolute)
becomes q (relative).

Opcodes 0x80 to 0x8f means N consecutive S ops, for N in [1, 16]. The opcode is
followed by 4*N coordinates, N sets of (x2, y2, x, y).

Opcodes 0x90 to 0x9f are like the previous paragraph, except S (absolute)
becomes s (relative).

Opcodes 0xa0 to 0xaf means N consecutive C ops, for N in [1, 16]. The opcode is
followed by 6*N coordinates, N sets of (x1, y1, x2, y2, x, y).

Opcodes 0xb0 to 0xbf are like the previous paragraph, except C (absolute)
becomes c (relative).

Opcodes 0xc0 to 0xcf means N consecutive A ops, for N in [1, 16]. The opcode is
followed by 6*N numbers, N sets of (rx, ry, xAxisRotation, flags, x, y). The
rx, ry, x and y numbers are coordinates. The xAxisRotation number is an angle
(a zero-to-one number being a fraction of 360 degrees). The flags are encoded
as a natural number. The 0x01 bit of the decoded natural number is the
...
0a 00 50 50 b0 b0
Metadata (5 bytes after the length). MID 0 (VIEWBOX), (-24, -24) to (+24, +24).
