...
// "ComicSans.ttf", but a face has a specific size, style, weight and hinting.
// For example, the 12pt and 18pt versions of Comic Sans are two different
// faces, even if derived from the same font file.
//
...
//
// A Drawer is not safe to use concurrently.
type Drawer struct {
...
func (d *Drawer) DrawString(s string) {
	...
	for i, c := range s {
		...
		if !ok {
			// TODO: falling back on the U+FFFD glyph the responsibility of the
			// Drawer or the Face?
			continue
		}
