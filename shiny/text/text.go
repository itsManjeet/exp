...
type Frame struct {
	...
	// empty-but-not-nil *Foo child. Every Frame contains at least one
	// Paragraph. Similarly, every Paragraph contains at least one Line, and
	// every every Line contains at least one Box.
	//
...
func (f *Frame) FirstParagraph() *Paragraph {
	if f.firstParagraph == 0 {
		// 0 means that the Paragraph is implicit, so we make it explicit.
...
func (p *Paragraph) FirstLine(f *Frame) *Line {
	if p.firstLine == 0 {
		// 0 means that the Line is implicit, so we make it explicit.
...
func (l *Line) FirstBox(f *Frame) *Box {
	if l.firstBox == 0 {
		// 0 means that the Box is implicit, so we make it explicit.
		l.firstBox = f.newBox()
	}
