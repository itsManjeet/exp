...
// GL is a widget that maintains an OpenGL ES context into which
// OpenGL calls can be rendered.
//
...
func NewGL(drawFunc func(*GL)) *GL {
	...
	w := &GL{
		Ctx:  gldriver.NewContext(),
		Draw: drawFunc,
	}
	tex := w.Ctx.CreateTexture()
	w.Ctx.BindTexture(gl.TEXTURE_2D, tex)
	...
	w.Ctx.FramebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, tex, 0)

	// TODO: delete the framebuffer and texture.
...
func (w *GL) Measure(t *theme.Theme) {
	w.MeasuredSize = image.Point{}
}

func (w *GL) Layout(t *theme.Theme) {
}
