...
func (k LabelClass) Measure(n *Node, t *Theme) {
	...
	// The 6 is because font metrics are in 26.6 fixed-point units.
	n.MeasuredSize.X = int(font.MeasureString(f, o.Text()).Ceil()) >> 6
	n.MeasuredSize.Y = int(m.Ascent.Ceil()+m.Descent.Ceil()) >> 6
}
