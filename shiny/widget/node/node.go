...
// Arity is the number of children a node can have.
type arity uint8

const (
	arityLeaf      = arity(0) // Leaf nodes have no children.
	arityShell     = arity(1) // Shell nodes have at most one child.
	arityContainer = arity(2) // Container nodes can have any number of children.
)
...
func (e *LeafEmbed) AppendChild(c Node) { e.appendChild(arityLeaf, c) }
...
func (e *ShellEmbed) AppendChild(c Node) { e.appendChild(arityShell, c) }
...
func (e *ContainerEmbed) AppendChild(c Node) { e.appendChild(arityContainer, c) }
...
func (n *Embed) Wrappee() *Embed { return n }

func (n *Embed) appendChild(a arity, c Node) {
	m := c.Wrappee()
	if m.Parent != nil || m.PrevSibling != nil || m.NextSibling != nil {
		panic("widget: AppendChild called for an attached child")
	}
	switch a {
	case arityLeaf:
		panic("widget: AppendChild called for a leaf parent")
	case arityShell:
		if n.FirstChild != nil {
			panic("widget: AppendChild called for a shell parent that already has a child")
		}
	}
	last := n.LastChild
	if last != nil {
...
func (n *Embed) removeChild(c Node) {
	m := c.Wrappee()
	if m.Parent != n {
