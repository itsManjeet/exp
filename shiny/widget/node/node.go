...
type Node interface {
	...
	// that PaintBase is recursively triggered by an ancestor node. If it does
	// create those RGBA pixel buffers, it is also responsible for calling
	// PaintBase on this node (and its children). In any case, the Paint method
	// should then paint any effects. Many widgets will neither create their
	// own buffers nor have any effects, so their Paint methods will simply be
	// the default implemention: do nothing except call Paint on its children.
	// As mentioned above, the bulk of the work is typically done in PaintBase.
	//
