...
func main() {
	...
	f := widget.NewFlow()
	f.SetAxis(widget.AxisVertical)
	f.AppendChild(mkImage(80, 40, color.RGBA{0xff, 0x00, 0x00, 0xff}))
	f.AppendChild(mkImage(50, 50, color.RGBA{0x00, 0xff, 0x00, 0xff}))
	f.AppendChild(mkImage(20, 60, color.RGBA{0x00, 0x00, 0xff, 0xff}))
	...
	f.Class.Measure(f.Node, t)
	f.Rect = rgba.Bounds()
	f.Class.Layout(f.Node, t)
	f.Class.Paint(f.Node, t, rgba)
