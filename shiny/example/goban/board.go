...
// isBlank reports whether the piece is a blank, that is, no piece at all.
func (p *Piece) isBlank() bool {
	return p == nil
}
...
func makeStone(name string, circleMask *image.Alpha) (*image.RGBA, *image.Alpha) {
	...
	Src := image.NewRGBA(stone.Bounds())
	// Make the whole area black, for the shadow.
	draw.Draw(Src, Src.Bounds(), image.Black, ZP, draw.Src)
	// Lay in the stone within the circle so it shows up inside the shadow.
	draw.DrawMask(Src, Src.Bounds(), stone, ZP, circleMask, ZP, draw.Over)
	return Src, makeShadowMask(stone)
}
...
func makeShadowMask(stone image.Image) *image.Alpha {
	...
	for y := 0; y < size; y++ {
		...
		for x := 0; x < size; x++ {
			...
			x2 *= x2
			yy := uint8(0)
			switch c := stone.At(x, y).(type) {
			case color.Gray16:
				yy = uint8(c.Y >> 8)
			case color.Gray:
				yy = c.Y
			default:
				log.Fatal("unknown color type %T", c)
			}
			if x2+y2 > stoneRad2 {
				Mask.SetAlpha(x, y, color.Alpha{255 - yy})
			} else {
				Mask.SetAlpha(x, y, color.Alpha{255})
			}
		}
...
func (b *Board) Draw(m *image.RGBA) {
	...
	for i := 1; i <= b.dim; i++ {
		for j := 1; j <= b.dim; j++ {
			ij := IJ{i, j}
			if !b.piece(ij).isBlank() {
				b.drawPiece(m, ij, b.piece(ij))
			}
		}
	}
}
...
func (b *Board) click(m *image.RGBA, x, y, button int) {
	...
	case 3:
		b.pieces[(ij.j-1)*b.Dims.dim+ij.i-1] = nil
	}
