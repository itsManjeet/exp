...
var (
	fontFlag      = flag.String("font", "", `filename of the Plan 9 font file, such as "lucsans/unicode.8.font"`)
	subfontFlag   = flag.String("subfont", "", `filename of the Plan 9 subfont file, such as "lucsans/lsr.14"`)
...
func main() {
	...
	// TODO: mmap the files.
	if *fontFlag == "" && *subfontFlag == "" {
		flag.Usage()
		log.Fatal("no font or subfont specified")
	}
	var face font.Face
	if *fontFlag != "" {
		...
		face, err = plan9font.ParseFont(fontData, func(name string) (io.ReadCloser, error) {
		...
		fontData, err := ioutil.ReadFile(*subfontFlag)
		if err != nil {
			log.Fatal(err)
		}
