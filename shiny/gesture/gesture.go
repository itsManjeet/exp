...
type Event struct {
	...
	// PressPos is the initial position of the button press or touch that
	// started this gesture.
	PressPos Point

	// CurPos is the current position of the button or touch event.
	CurPos Point
...
func (f *EventFilter) sendLater(e internalEvent, sleep time.Duration) {
	time.Sleep(sleep)
...
func (f *EventFilter) Filter(e interface{}) interface{} {
	...
	case internalEvent:
		...
		case typeDoublePressSchedule:
			...
			go f.sendLater(e, doublePressThreshold)
		...
		case typeLongPressSchedule:
			...
			go f.sendLater(e, longPressThreshold)
	...
	case mouse.Event:
		...
		case mouse.DirNone:
			...
			startDrag := false
			if !f.drag &&
				(abs(e.X-f.pressPos.X) > dragThreshold || abs(e.Y-f.pressPos.Y) > dragThreshold) {
				f.drag = true
				startDrag = true
			}
			if f.drag {
