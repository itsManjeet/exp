...
func (c *core) Write(e zapcore.Entry, fs []zapcore.Field) error {
	b := c.builder.Clone()
	b.Event.At = e.Time
	b.With(internal.LevelKey.Of(int(e.Level))) // TODO: convert zap level to general level
	b.With(internal.NameKey.Of(e.LoggerName))
...
func newLabel(f zap.Field) event.Label {
	...
	case zapcore.BoolType:
		...
		return keys.Boolean(f.Key).Of(b)
