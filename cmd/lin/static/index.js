...
const redrawEdgelist = graph => {
  ...
  Object.entries(graph).forEach(entry => {
    ...
    for (const to in tos) {
      ...
      rowButton.onclick = _ => {
        fetch('/edge', {method: 'DELETE', body: JSON.stringify({'from': from, 'to': to})}).then(function(resp) {
          resp.json().then(both => {
      ...
      newEdgeRow.onmouseout = _ => {
        newEdgeRow.style.backgroundColor = 'transparent'
        d3.select('svg')
          .selectAll('path')
          .filter(svgE => svgE.v == from && svgE.w == to )
          .each(function() {
...
const redrawShoppingCart = shoppingCart => {
  ...
  Object.entries(shoppingCart).forEach(entry => {
    const from = entry[0]
    const tos = entry[1]
    for (const to in tos) {
      const newEdgeRow = document.createElement('div')
      ...
      rowButton.className = 'right'
      rowButton.onclick = _ => {
        fetch('/edge', {method: 'POST', body: JSON.stringify({'from': from, 'to': to})}).then(function(resp) {
          resp.json().then(both => {
            redrawGraph(both['graph'])
            redrawEdgelist(both['graph'])
            redrawShoppingCart(both['shoppingCart'])
