...
func newRequestWithContext(ctx context.Context, method, url string) *http.Request {
	...
	// This is IMPORTANT. Fetching everything from the index without this header
	// would cause a number of upstream fetches, which would be extremely slow,
	// and would quickly lead to DoS protection kicking in.
...
func main() {
	...
	var noGoCode, noGoMod, gcsRedirect, good, goBytes, allBytes, goFiles int64
	for path, version := range latestVersions {
		...
		g.Go(func() error {
			...
			// TODO: offer a flag to filter fewer modules.
			if !hasGoMod {
			...
			for _, f := range z.File {
				if !strings.HasSuffix(f.Name, ".go") &&
					// TODO: maybe include .s, .syso, and cgo files?
					!strings.HasSuffix(f.Name, "/go.mod") &&
					!strings.HasSuffix(f.Name, "/go.sum") {
					continue
				}
				...
				name := filepath.Join(flag.Arg(0), f.Name)
