...
func writeText(r *vulncheck.Result, pkgs []*packages.Package) {
	...
	for _, vg := range vulnGroups {
		...
		fns := representativeFuncs(vg, topModules, callStacks)
...
func representativeFuncs(vg []*vulncheck.Vuln, topmods map[string]bool, callStacks map[*vulncheck.Vuln][]vulncheck.CallStack) []*vulncheck.FuncNode {
