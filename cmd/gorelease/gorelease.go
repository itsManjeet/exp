...
func main() {
	...
	success, err := runRelease(context.Background(), os.Stdout, wd, os.Args[1:])
	if err != nil {
...
func loadLocalModule(ctx context.Context, modRoot, repoRoot, version string) (m moduleInfo, err error) {
	...
	tmpLoadDir, tmpGoModData, tmpGoSumData, pkgPaths, prepareDiagnostics, goSumMissing, err := prepareLoadDir(ctx, m.goModFile, m.modPath, tmpModRoot, version, false)
	if err != nil {
		return moduleInfo{}, err
	}
	defer func() {
	...
	// We can't run "go list" if the go.sum is missing or incomplete.
	if !goSumMissing {
		ev, err := existingVersions(ctx, m.modPath, m.modRoot)
		if err != nil {
			return moduleInfo{}, err
		}
...
func loadDownloadedModule(ctx context.Context, modPath, version, max string) (m moduleInfo, err error) {
	...
	// Load packages.
	tmpLoadDir, tmpGoModData, tmpGoSumData, pkgPaths, _, _, err := prepareLoadDir(ctx, nil, m.modPath, m.modRoot, m.version, true)
	if err != nil {
		return moduleInfo{}, err
	}
	defer func() {
...
func prepareLoadDir(ctx context.Context, modFile *modfile.File, modPath, modRoot, version string, cached bool) (dir string, goModData, goSumData []byte, pkgPaths []string, diagnostics []string, goSumMissing bool, err error) {
	defer func() {
		if err != nil {
			if cached {
	...
	if err != nil {
		return "", nil, nil, nil, nil, goSumMissing, err
	}
	...
	if _, err := cmd.Output(); err != nil {
		return "", nil, nil, nil, nil, goSumMissing, fmt.Errorf("looking for missing dependencies: %w", cleanCmdError(err))
	}
	...
	if len(missing) > 0 {
		...
		return dir, goModData, goSumData, imps, diagnostics, true, nil
	}
	...
	return dir, goModData, goSumData, imps, diagnostics, goSumMissing, nil
}
