...
const helpText = `gorelease is an experimental tool that helps module authors avoid common
problems before releasing a new version of a module.
...
gorelease accepts the following flags:
	...
	-base=version
		The version that the current version of the module will be compared
		against. The version must be a semantic version (for example, "v2.3.4")
		or "none". If the version is "none", gorelease will not compare the
		current version against any previous version; it will only validate the
		current version. This is useful for checking the first release of a new
		major version.

	-version=version
		The proposed version to be released. If specified, gorelease will
		confirm whether this version is consistent with changes made to the
		module's public API. gorelease will exit with a non-zero status if the
...
func runRelease(w io.Writer, dir string, args []string) (success bool, err error) {
	...
	fs.Usage = func() {}
	fs.SetOutput(ioutil.Discard) // we'll print our own errors.
	var baseVersion, releaseVersion string
	fs.StringVar(&baseVersion, "base", "", "previous version that the module will be compared against")
