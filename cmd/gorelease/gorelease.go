...
type moduleInfo struct {
	...
	dlModPath       string // module path used to download module (may differ from modPath in forks)
...
//
// modPath is the module path used to fetch the module (returned in
// m.dlModPath). The module's path in go.mod (m.modPath) may be different,
// for example in a soft fork intended as a replacement.
//
...
func loadDownloadedModule(modPath, version, max string) (m moduleInfo, err error) {
	...
	m = moduleInfo{modPath: modPath, dlModPath: modPath}
