...
func makeReleaseReport(ctx context.Context, base, release moduleInfo) (report, error) {
	...
	vs := existingVersions(ctx, base.modPath, base.modRoot)
	if r.canVerifyReleaseVersion() {
		if release.version == "" {
			r.suggestReleaseVersion(vs)
		} else {
			r.validateReleaseVersion(vs)
		}
	}
...
// existingVersions returns the versions that already exist for the given modPath.
func existingVersions(ctx context.Context, modPath, modRoot string) []string {
