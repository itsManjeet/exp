...
func prepareLoadDir(modFile *modfile.File, modPath, modRoot, version string) (dir string, goModData, goSumData []byte, pkgPaths []string, err error) {
	...
	// requirements.
	fakeImports := []string{}
	pkgs, err := collectImports(modPath, modRoot)
	if err != nil {
	...
	for _, pkg := range pkgs {
		...
		fakeImports = append(fakeImports, fmt.Sprintf("import _ %q", pkg))
	}
	...
	return dir, goModData, goSumData, pkgs, nil
}
...
func collectImports(modPath, root string) (imports []string, _ error) {
	...
	err := filepath.Walk(root, func(path string, fi os.FileInfo, err error) error {
		...
		// Skip main because external users can't import a main package, so
		// gorelease should not consider any main package information (including
