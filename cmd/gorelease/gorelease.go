...
func prepareLoadDir(modFile *modfile.File, modPath, modRoot, version string) (dir string, goModData, goSumData []byte, err error) {
	...
	// require statements.
	fakeImports := []string{fmt.Sprintf("import _ %q", modPath)}
	if modFile != nil {
		for _, r := range modFile.Require {
			if r.Indirect {
				continue
			}
			fakeImports = append(fakeImports, fmt.Sprintf("import _ %q", r.Mod.Path))
		}
	}
...
func loadPackages(modPath, modRoot, loadDir string, goModData, goSumData []byte) (pkgs []*packages.Package, diagnostics []string, err error) {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "updates to go.sum needed") || strings.Contains(err.Error(), "missing go.sum entry") {
