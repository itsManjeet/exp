...
//
// Note: gorelease does not accept build metadata in releases. Although it is
// valid semver, the Go tool and other tools in the ecosystem do not support it,
...
func runRelease(w io.Writer, dir string, args []string) (success bool, err error) {
	...
	if releaseVersion != "" {
		if semver.Build(releaseVersion) != "" {
			w.Write([]byte(fmt.Sprintf("release version %q is not a canonical semantic version", releaseVersion)))
			return true, nil
		}
