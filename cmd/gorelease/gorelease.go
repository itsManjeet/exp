...
func loadLocalModule(modRoot, repoRoot, version string) (m moduleInfo, err error) {
	...
	tmpLoadDir, tmpGoModData, tmpGoSumData, pkgPaths, err := prepareLoadDir(m.goModFile, m.modPath, tmpModRoot, version, false)
	if err != nil {
		return moduleInfo{}, err
	}
	defer func() {
...
func loadDownloadedModule(modPath, version, max string) (m moduleInfo, err error) {
	...
	// Load packages.
	tmpLoadDir, tmpGoModData, tmpGoSumData, pkgPaths, err := prepareLoadDir(nil, m.modPath, m.modRoot, m.version, true)
	if err != nil {
		return moduleInfo{}, err
	}
	defer func() {
...
// If true, the module lives in modRoot and can be referenced with a simple
// requirement.
// If false, the module lives in
// $GOMODCACHE/cache/download/$modPath/@v/$version.mod and will be referenced
// with a local replace directive.
//
...
//
// prepareLoadDir creates a temporary directory with a go module that requires
// the given modPath. It also creates a .go file that imports every package in
// the given modPath. This temporary module is useful for two reasons. First,
// replace and exclude directives from the target module aren't applied, so we
// have the same view as a dependent module. Second, we can run commands like
// 'go get' without modifying the original go.mod and go.sum files.
func prepareLoadDir(modFile *modfile.File, modPath, modRoot, version string, cached bool) (dir string, goModData, goSumData []byte, pkgPaths []string, err error) {
	defer func() {
		if err != nil {
			if cached {
	...
	for _, imp := range imps {
		...
		fmt.Fprintf(fakeImports, "import _ %q", imp.importPath)
	}
	...
	if _, err := cmd.Output(); err != nil {
		return "", nil, nil, nil, fmt.Errorf("looking for missing dependencies: %w", cleanCmdError(err))
	}
	...
	// TODO(deklerk): Why do we skip the comparison? Because it's outside of the
	// user's control? (it's a downloaded module)
	if !cached && bytes.Compare(goSumData, newGoSumData) != 0 {
...
func collectImports(modPath, root string) (imports []importFromMod, _ error) {
	err := filepath.Walk(root, func(path string, fi os.FileInfo, err error) error {
		...
		pathFromRoot := strings.TrimPrefix(p.Dir, root)
