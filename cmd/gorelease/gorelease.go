...
// However, if the proposed version has major version 0, gorelease may exit
// with a zero status even if there are incompatible changes: there is no
// strong expectation of compatibility at v0.
//
...
func makeReleaseReport(modRoot, repoRoot, baseVersion, releaseVersion string) (report, error) {
	...
	if releaseVersion != "" {
		r.versionErr = r.validateVersion()
	} else {
		r.releaseVersion, r.versionErr = r.suggestVersion()
