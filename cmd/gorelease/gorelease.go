...
// example, if incompatible changes are made in the module "example.com/mod", a
// new major version must be released as a new module, "example.com/mod/v2".
//
// gorelease accepts both compatible and incompatible changes when the
// proposed version has major version number 0. There is no strong expectation
// of compatibility at v0.
//
...
//   or increments the minor version without zeroing the patch.
...
func makeReleaseReport(modRoot, repoRoot, baseVersion, releaseVersion string) (report, error) {
	...
	if modPath == "" {
		return report{}, fmt.Errorf("%s: could not read module path", goModPath)
	}
	...
	for _, pair := range zipPackages(basePkgs, releasePkgs) {
		...
		default:
			...
			if !isInternal(basePkg.PkgPath) && basePkg.Name != "main" && releasePkg.Name != "main" {
				...
				if len(basePkg.Errors) == 0 && len(releasePkg.Errors) == 0 {
					pr.Report = apidiff.Changes(basePkg.Types, releasePkg.Types)
				}
				r.addPackage(pr)
			}
		}
