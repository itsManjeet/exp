...
func makeReleaseReport(ctx context.Context, base, release moduleInfo) (report, error) {
	...
	vs := priorVersions(ctx, base.modPath, base.modRoot)

	if r.canVerifyReleaseVersion() {
		if release.version == "" {
			r.suggestReleaseVersion(vs)
		} else {
	...
	for _, v := range vs {
		if semver.Compare(v, r.release.version) == 0 {
			r.release.diagnostics = append(r.release.diagnostics, fmt.Sprintf("version %s already exists", v))
		}
	}
...
func priorVersions(ctx context.Context, modPath, modRoot string) []string {
	// TODO(deklerk): Clean up this section properly.
...
// semver order.
func loadVersions(ctx context.Context, modPath string) ([]string, error) {
	tmpDir, err := ioutil.TempDir("", "")
	if err != nil {
		return nil, err
	}
