...
func prepareProxy(proxyVersions map[module.Version]bool) (ctx context.Context, cleanup func(), _ error) {
	env := append(os.Environ(), "GO11MODULE=on", "GOSUMDB=off")
...
func TestRelease(t *testing.T) {
	defaultContext, cleanup, err := prepareProxy(nil)
	if err != nil {
	...
	for _, testPath := range testPaths {
		...
		t.Run(testName, func(t *testing.T) {
			...
			if len(test.proxyVersions) > 0 {
				var cleanup func()
				ctx, cleanup, err = prepareProxy(test.proxyVersions)
				if err != nil {
					t.Errorf("preparing test proxy: %v", err)
				}
				defer cleanup()
			}
