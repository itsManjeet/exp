...
import (
	...
	"golang.org/x/exp/cmd/gorelease/internal/lockedfile"
	"golang.org/x/exp/cmd/gorelease/internal/par"
	"golang.org/x/exp/cmd/gorelease/internal/semver"
)
...
func (r *gitRepo) DescendsFrom(rev, tag string) (bool, error) {
	...
	if ee, ok := err.(*RunError).Err.(*exec.ExitError); ok && ee.Exited() {
		return false, nil
	}
	return false, err
}
