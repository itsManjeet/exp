...
//   - A requirement was increment to a new minor version.
...
func (r *report) requirementsChanged() bool {
	...
	for _, r := range r.base.goModFile.Require {
		baseReqs[r.Mod.Path] = semver.MajorMinor(r.Mod.Version)
	}
	...
	for _, r := range r.release.goModFile.Require {
		...
		if semver.MajorMinor(r.Mod.Version) > baseReqs[r.Mod.Path] {
