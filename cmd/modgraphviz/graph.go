...
//
// TODO(deklerk): jayconrod@ notes the follwing in https://go-review.googlesource.com/c/exp/+/172969:
//
// I think this can be optimized to an algorithm with better complexity.
// Currently, it can visit a node multiple times when it's reachable along
// multiple paths.
//
// It should be possible to visit each vertex only once with a depth first
// search. Keep a set of vertices from which needle is reachable. In post-order,
// a vertex should be added to this set if it is needle or if one of its
// neighbors is in the set. After the traversal, you can construct the new graph
// with that set of vertices and the edges between those vertices.
func (g *graph) to(needle string) (*graph, error) {
	...
	for cursor := q[0]; len(q) > 0; cursor, q = q[0], q[1:] {
		...
		if cursor.name == needle {
			...
			// The path is sorted needle - ... - root, so let's traverse
			// backwards to add them in root - ... - needle order.
