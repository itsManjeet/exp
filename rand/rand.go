...
// Sources created by NewSource are not. However, Sources are small and it is
// reasonable to have a separate Source for each goroutine, seeded differently, to
// avoid locking.
//
...
func (r *lockedSource) Seed(seed uint64) {
	r.lk.Lock()
	r.src.Seed(seed)
	r.lk.Unlock()
}
...
func (r *lockedSource) seedPos(seed uint64, readPos *int8) {
	r.lk.Lock()
	r.src.Seed(seed)
	*readPos = 0
	r.lk.Unlock()
...
func (r *lockedSource) read(p []byte, readVal *uint64, readPos *int8) (n int, err error) {
	r.lk.Lock()
	n, err = read(p, r.src.Uint64, readVal, readPos)
	r.lk.Unlock()
	return
}
