...
func Int64(key string, value int64) Attr {
	return Attr{key: key, u: uint64(value), a: Int64Kind}
}
...
func Uint64(key string, value uint64) Attr {
	return Attr{key: key, u: value, a: Uint64Kind}
}

// Float64 returns an Attr for a floating-point number.
func Float64(key string, value float64) Attr {
	return Attr{key: key, u: math.Float64bits(value), a: Float64Kind}
}
...
func Bool(key string, value bool) Attr {
	...
	return Attr{key: key, u: u, a: BoolKind}
}

// Time returns an Attr for a time.Time.
func Time(key string, value time.Time) Attr {
	return Attr{key: key, u: uint64(value.UnixNano()), a: value.Location()}
}

// Duration returns an Attr for a time.Duration.
func Duration(key string, value time.Duration) Attr {
	return Attr{key: key, u: uint64(value.Nanoseconds()), a: DurationKind}
}

// Any returns an Attr for the supplied value.
//
// Any does not preserve the exact type of integral values.
// All signed integers are converted to int64 and all
// unsigned integers to uint64.
// Similarly, float32s are converted to float64.
//
// However, named types are preserved.
// So given
//
//	type Int int
//
...
func Any(key string, value any) Attr {
	...
	case Kind:
		panic("cannot store a kv.Kind in an Attr")
	default:
		return Attr{key: key, a: v}
	}
}
...
func (a Attr) HasValue() bool { return a.a != nil }
...
func (a Attr) duration() time.Duration {
	return time.Duration(int64(a.u))
}
...
func (a Attr) float() float64 {
	return math.Float64frombits(a.u)
}
...
func (a Attr) time() time.Time {
	return time.Unix(0, int64(a.u)).In(a.a.(*time.Location))
}
...
func (a Attr) AppendValue(dst []byte) []byte {
	...
	case Int64Kind:
		return strconv.AppendInt(dst, int64(a.u), 10)
	case Uint64Kind:
		return strconv.AppendUint(dst, a.u, 10)
	case Float64Kind:
	...
	case AnyKind:
		return append(dst, fmt.Sprint(a.a)...)
	default:
		panic("bad kind")
	}
}
