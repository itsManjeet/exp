...
func TestAny(t *testing.T) {
	...
	if Any[any]("k", nil).Value() != nil {
		t.Error("wanted nil")
	}
}

// Compare Any[T](T) with Any(any).
...
func nonGenericAny(key string, val any) Attr {
	a := Attr{key: key}
	a.setAnyValue(val)
	return a
}
